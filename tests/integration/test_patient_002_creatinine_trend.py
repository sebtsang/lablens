"""End-to-end integration test for Patient 2 (creatinine trend, CLAUDE.md §10)."""

from __future__ import annotations

import json
from pathlib import Path
from typing import Any, cast

import pytest
from anthropic import Anthropic

from lablens.fhir.client import FhirClient
from lablens.fhir.queries import fetch_lab_trend, fetch_patient_context
from lablens.tools.analyze_medication_lab_interactions import run as analyze
from lablens.tools.classify_lab_followup_urgency import classify
from tests.fixtures.fake_anthropic import FakeAnthropic
from tests.fixtures.fake_fhir_client import FakeFhirClient

BUNDLE_PATH = (
    Path(__file__).resolve().parents[2]
    / "data"
    / "synthetic_patients"
    / "patient_002_creatinine_trend.json"
)
PATIENT_ID = "lablens-patient-002"

LLM_RESPONSE = json.dumps(
    {
        "mechanisms": [
            {
                "medication": "Losartan 100 mg oral tablet",
                "summary": "ARB blockade of angiotensin II reduces glomerular filtration pressure.",
            },
            {
                "medication": "Naproxen 500 mg oral tablet",
                "summary": "NSAID prostaglandin inhibition reduces afferent arteriolar dilation, decreasing renal perfusion.",
            },
        ],
        "cumulative_risk_note": (
            "ARB + NSAID combination synergistically compromises glomerular filtration, "
            "explaining the acute creatinine rise."
        ),
    }
)


def _split_bundle(
    bundle_path: Path, patient_id: str
) -> tuple[
    dict[str, dict[str, Any] | None],
    dict[tuple[str, frozenset[tuple[str, str]]], dict[str, Any] | None],
]:
    raw = json.loads(bundle_path.read_text())
    entries: list[dict[str, Any]] = raw.get("entry", [])
    by_type: dict[str, list[dict[str, Any]]] = {}
    patient_resource: dict[str, Any] | None = None
    for entry in entries:
        resource = entry.get("resource", {})
        rt = resource.get("resourceType")
        if rt == "Patient":
            patient_resource = resource
        else:
            by_type.setdefault(rt, []).append(resource)

    def bundle_of(resources: list[dict[str, Any]]) -> dict[str, Any] | None:
        return {
            "resourceType": "Bundle",
            "type": "searchset",
            "entry": [{"resource": r} for r in resources],
        }

    reads: dict[str, dict[str, Any] | None] = {f"Patient/{patient_id}": patient_resource}
    searches: dict[tuple[str, frozenset[tuple[str, str]]], dict[str, Any] | None] = {
        ("Condition", frozenset()): bundle_of(by_type.get("Condition", [])),
        ("MedicationRequest", frozenset()): bundle_of(by_type.get("MedicationRequest", [])),
        ("AllergyIntolerance", frozenset()): bundle_of(by_type.get("AllergyIntolerance", [])),
        ("Encounter", frozenset()): bundle_of(by_type.get("Encounter", [])),
        ("Observation", frozenset()): bundle_of(by_type.get("Observation", [])),
    }
    return reads, searches


@pytest.mark.asyncio
async def test_patient_002_creatinine_trend_full_pipeline() -> None:
    reads, searches = _split_bundle(BUNDLE_PATH, PATIENT_ID)
    fhir = cast("FhirClient", FakeFhirClient(reads=reads, searches=searches))
    anth = cast("Anthropic", FakeAnthropic(LLM_RESPONSE))

    patient_context = await fetch_patient_context(fhir, PATIENT_ID)
    classes = {m.drug_class for m in patient_context.active_medications}
    assert {"ARB", "NSAID"}.issubset(classes)

    lab_trend = await fetch_lab_trend(fhir, PATIENT_ID, "CREATININE")
    assert lab_trend.values[0].value == 2.1
    assert lab_trend.delta_from_baseline is not None
    assert lab_trend.delta_from_baseline >= 0.5  # Demonstrates large delta

    interactions = analyze("CREATININE", patient_context.active_medications, anthropic_client=anth)
    assert interactions.has_significant_interactions is True
    classes_found = {i.drug_class for i in interactions.interactions_found}
    assert classes_found == {"ARB", "NSAID"}

    result = classify(
        lab_type="CREATININE",
        current_value=2.1,
        unit="mg/dL",
        patient_context=patient_context,
        lab_trend=lab_trend,
        medication_interactions=interactions,
    )
    assert result.urgency == "URGENT"
    assert "4 hours" in result.recommended_review_path
    # Expect either the rising-trend rule or the large-delta rule to fire
    assert any(("RISING" in t) or ("delta" in t.lower()) for t in result.rule_trace)
