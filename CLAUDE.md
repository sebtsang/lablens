# LabLens v3 — Claude Code Handoff Document
...
**Audience for this document:** Claude Code (or another agentic coding assistant) acting as the implementation partner for this team.

---

## 0. Pre-flight: workspace setup (do this FIRST, before reading the rest)

This document assumes you are running inside a project folder that has been initialized as a Git repository. If you are NOT — i.e., the team just opened Claude Code in an empty folder and pointed you at this file — your very first job is to set up the workspace correctly. Walk through these steps in order. Do NOT skip ahead to clinical implementation.

### 0.1 Verify you are in the right folder

Run `pwd` and `ls -la`. You should be in a folder named `lablens` (or similar) and you should see this `CLAUDE.md` file. If you don't see `CLAUDE.md` in the listing, stop and ask the team where to find it.
...
- **LLM step (Claude, GPT, or Gemini — pick one and stick with it):** For each identified interaction, generate the `mechanism_summary` field using a tightly constrained prompt (see section 8). For the `cumulative_risk_note`, generate a 1-2 sentence summary of the overall medication-driven risk picture.
...
**Model choice:** Use Claude (claude-sonnet-4-6 or claude-opus-4-7) via the Anthropic API. Set temperature to 0.2. Max tokens 800.
